...
class Student_action:
    ...
    def add_student(self):
        rows = self.he.get_rows(11)
        try:
            for i in range(1, int(rows) + 1):
                is_run = self.he.get_value(i, 4, 11)
                if is_run == "yes":
                    action_ways = self.he.get_value(i, 5, 11)
                    input_data = self.he.get_value(i, 6, 11)
                    # print(input_data)
                    page = self.he.get_value(i, 7, 11)
                    element = self.he.get_value(i, 8, 11)
                    Expect_page = self.he.get_value(i, 9, 11)
                    Expect_element = self.he.get_value(i, 10, 11)
                    EXpect_result = self.he.get_value(i, 11, 11)
                    element_number = self.he.get_value(i, 13, 11)
                    pre_condition = self.he.get_value(i, 14, 11)
